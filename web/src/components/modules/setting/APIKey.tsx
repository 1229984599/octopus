...
function APIKeyFormOverlay({
    ...
    // 不用 layoutId morph：shared layout + CSS translate 会卡在按钮尺寸缩放(~0.08)
    // 用 fixed 全屏 flex 居中，避免 absolute 矮父裁切、也避免 transform 冲突
    return (
        <motion.div
        ...
        >
            <motion.div
                ...
                className="w-[min(420px,calc(100vw-2rem))] bg-card p-5 rounded-3xl border border-border max-h-[80vh] overflow-auto shadow-xl"
    ...
    return (
        ...
        >
            <motion.div
                ...
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95, transition: { duration: 0.15 } }}
                className="w-[min(320px,calc(100vw-2rem))] flex flex-col bg-card p-5 rounded-3xl border border-border max-h-[80vh] overflow-auto shadow-xl"
                transition={{ type: 'spring', stiffness: 400, damping: 30 }}
                onClick={(e) => e.stopPropagation()}
            >
...
function APIKeyPanelBase({
    ...
    // form/stats overlay 已不用 shared layout morph；仅删除确认仍用 layoutId
