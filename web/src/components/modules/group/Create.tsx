        ...
        <div className="flex h-[calc(100vh-2rem)] min-h-0 w-[calc(100vw-1.5rem)] max-w-full flex-col md:w-screen md:max-w-4xl">
            ...
            <MorphingDialogDescription className="min-h-0 flex-1 overflow-hidden">
                <GroupEditor
                    ...
                    onSubmit={({ name, match_regex, mode, first_token_time_out, session_keep_time, auto_check, members }) => {
                        ...
                        createGroup.mutate(
                            { name, mode, match_regex: match_regex ?? '', first_token_time_out: first_token_time_out ?? 0, session_keep_time: session_keep_time ?? 0, auto_check, items },
                            {
